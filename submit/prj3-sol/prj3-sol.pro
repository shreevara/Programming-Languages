...
dept_employees(Employees, Dept,DeptEmployees):-
  rev(Employees, Dept, [], DeptEmployees).

rev([], _, Acc,Acc).
rev([X|Xs], Dept, Acc, Zs):-
    X=employee(_,_,S,_),
    Dept == S -> append(Acc,[X],C), rev(Xs,Dept,C,Zs);
    rev(Xs,Dept,Acc,Zs).
...
access([X|Xs],L,O):-
    Xs\=[] -> check(X,L,Ys), C = Ys, access(Xs,C,O);
    check(X,L,O).

list_access([],L,L).
list_access([X|Xs],L,Zs):-
    X == [] -> Zs is L;
    access([X|Xs],L,O),
    O \= false -> Zs = O;
    Zs = nil.
...
count_lists(E, R):-
    maplist(count_lists, E, Cs) -> sum_list(Cs, S), R is S+1 ;
    R = 0.

flatten([],[]).
flatten([Head|Tail],R) :-
	flatten(Head,New_Head),flatten(Tail,New_Tail),
...
re_match(conc(Rx1, Rx2), List) :-
    append(List1, List2, List),  re_match(Rx2, List2),  re_match(Rx1, List1).

re_match(alt(Rx1, Rx2), List) :-
    re_match(Rx1, List); re_match(Rx2, List).

re_match(kleene(Rx), List) :- append([H|List1], List2, List),
                            re_match(Rx, [H|List1]),
                            re_match(kleene(Rx), List2).
...
rules(P, RuleClause):-
    atomic(P) ->  RuleClause = P;
    rules(P, [], RuleClause).

rules(P, Acc,RuleClause):-
    P = (T:-S),compound(P) ->  rules(S, T,RuleClause );
    P = (Q,R) ->   Acc1 = Acc \/ ~Q, temp(R, Acc1,RuleClause);
	atomic(Acc) -> RuleClause=P;
    P = (T:-S), \+ compound(P) ->  rules(S, T,RuleClause ).


rules(R, Acc, RuleClause):-
    compound(R),compound(Acc), R\=(A,B), Acc\=(C,D) ->  RuleClause = Acc \/ ~R.

temp(R, Acc, RuleClause):-
    R = (_,_) ->  rules(R,Acc,RuleClause);
    RuleClause = Acc \/ ~R.
