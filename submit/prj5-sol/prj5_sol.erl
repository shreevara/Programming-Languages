...
dept_employees1(Dept, Employees) -> dept_employees1_rec(Dept,Employees,[]).

dept_employees1_rec(Dept, [], Acc) -> Acc;
dept_employees1_rec(Dept, [H|T], Acc) -> 
  if H#employee.dept == Dept ->
    dept_employees1_rec(Dept, T, Acc ++ [H]);
  true ->
    dept_employees1_rec(Dept, T, Acc)
  end.
...
filter(false, _H, P, T) -> filter(P, T).
...
% [E] appended.
upsert_employee(E, Employees) -> upsert_employee_rec(E,Employees,[],0).

upsert_employee_rec(E, [], Acc,1) -> Acc;
upsert_employee_rec(E, [], Acc,0) -> Acc ++ [E];

upsert_employee_rec(E, [H|T], Acc, Flag) -> 
  if E#employee.name == H#employee.name ->
    upsert_employee_rec(E, T, Acc ++ [E],1);
  true ->
    upsert_employee_rec(E, T, Acc ++ [H],Flag)
  end.
...
find_employees(Preds, Employees) -> 
  ...
  [X || X <- Employees, all(Predicate, [all(Pred,[X]) || Pred <- Preds] )].
...
employees_req(Req, Employees) -> 
  case Req of
    ...
    { read , Name } ->
      Answer = find_employees([employee_has_name(Name)], Employees),
      if Answer == [] ->
        {err, lists:concat([Name, ' not found']) , Employees};
      true ->
        [H|T] = Answer,
        {ok, H, Employees}
      end;
...
employees_req_with_sort(Req, Employees) -> 
  case Req of
    { sort } -> 
      {ok, void, lists:keysort(#employee.name, Employees) };

    Other ->
      employees_req(Other, Employees)
  end.
...
start_employees_server(Employees, Fn) -> 
  register(prj5_sol, emps = spawn(fun() -> employees_server(Employees, Fn) end)),
  emps.
